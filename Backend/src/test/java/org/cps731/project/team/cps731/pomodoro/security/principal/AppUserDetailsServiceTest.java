...
public class AppUserDetailsServiceTest {
    ...
    public void shouldReturnAppUserDetailsWithStudentRoleForStudentsID() {
        var userID = 1L;
        var user = new User(userID, "John", "john.smith@torontomu.ca", "password", UserType.STUDENT);
        user.setId(userID);
    ...
    public void shouldReturnAppUserDetailsWithProfessorRoleForProfessorsID() {
        var userID = 1L;
        var user = new User(userID, "Jane Smith", "jane.smith@torontomu.ca", "password", UserType.PROFESSOR);
        user.setId(userID);
        when(userRepo.findById(userID))
                .thenReturn(Optional.of(user));
